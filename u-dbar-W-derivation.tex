...
\section{Full $ u \bar{d} \rightarrow W^+ \rightarrow e^+ \nu_e$ amplitude derivation }
...
The W$^+$ propagator is represented as, $$ \frac{-ig_{\mu\nu} +(\xi - 1)p_\mu p_\nu /(p^2 - \xi M_W^2)}{p^2 - M_W^2 +i\epsilon}.$$ For Standard Model processes, the two main gauges used are the R$_\xi$ gauge and the Unitary gauge; the latter is the limit as $\xi$ goes to infinity, of the R$_\xi$ gauge\cite{Wu:2016ljg}. Choosing the Feynman gauge ($\xi = 1$) results in a more simple expression for the propagator, $$\frac{-ig_{\mu\nu}}{p^2 - M_W^2 +i\epsilon}.$$ Fixing the gauge also introduces new terms in the Lagrangian; deriving the Feynman rules for this modified Lagrangian includes propagators for Goldstone Bosons and Faddeev-Popov ghosts, and interaction vertices \cite{Peskin:1995ev}. Goldstone Bosons and Faddeev-Popov ghosts are unphysical, yet, must be included in higher-order loop diagrams.\\
	
Choosing the Unitary gauge ($\xi \rightarrow \infty$), removes the Goldstone fields from the Lagrangian, and only physical fields remain \cite{Peskin:1995ev}. This leaves the W propagator as, $$ \frac{-i(g_{\mu\nu}+p_\mu p_\nu/ M_W^2 ) }{p^2 - M_W^2 +i\epsilon}.$$ Tree-level diagrams do not contain the unphysical ghost fields and Goldstone bosons; Sec (\textit{insert here}) shows how choosing the Unitary gauge gives the same result as if one chose the Feynman gauge. In order to get a precise result, one needs to take into account the higher order loops via Dyson resummation. This technique is used to sum an infinite subset of Feynman diagrams in a perturbative series to get a finite and physical result. This leads to an addition of the factor $iM_W\Gamma_W$ in the denominator of W propagator; in the Feynman gauge, $$\frac{-ig_{\mu\nu}}{p^2 - M_W^2 +iM_W\Gamma_W}.$$
...
\begin{equation}\label{complex-full-amp}
	\mathcal{M}^\dagger = -i \frac{G_FM_W^2 }{\sqrt{2}}\frac{\bar{v}_3 \overline{\left[\gamma_\mu (1-\gamma^5)\right]}u_4\bar{u}_1 \overline{\left[\gamma^\mu (1-\gamma^5)\right]}v_2}{p^2 - M_W^2- iM_W\Gamma_W}.
\end{equation}
...
\begin{equation}\label{squared-full-amp}
	\left(\frac{G_FM_W^2}{\sqrt{2}}\right)^2|V_\text{ud}|^2 \frac{\left( \bar{v}_3 \overline{\left[\gamma_\mu (1-\gamma^5)\right]}u_4\bar{u}_1 \overline{\left[\gamma^\mu (1-\gamma^5)\right]}v_2\right) \left( \bar{v}_2\left[\gamma^\mu(1-\gamma^5)\right]u_1\bar{u}_4 \left[\gamma_\mu(1-\gamma^5)\right]v_3\right)}{p^2+M_W^2)^2 +M_W^2\Gamma_W^2}.
\end{equation}

\subsection{Summing over spin states for hadronic and leptonic currents}
The hadronic current defined as, $$ H^{\alpha\mu} = \bar{u}_1 \left[\gamma^\mu (1-\gamma^5)\right]v_2\right)\bar{v}_2\left[\gamma^\mu(1-\gamma^5)\right]u_1,$$
and the leptonic current is defined as, $$L_{\alpha\mu}= \bar{v}_3 \left[\gamma_\mu (1-\gamma^5)\right]u_4\bar{u}_4 \left[\gamma_\mu(1-\gamma^5)\right]v_3.$$
...
\end{equation}
Now, substituting Eq.\ref{currents-contracted} back into Eq.\ref{spin-sum-sq-full-amp} gives the full spin-summed, squared amplitude,

\begin{equation}\label{final-total-amp}
	\sum_{s_1,s_2,s_3,s_4}|\mathcal{M}|^2 = \left(\frac{G_FM_W^2}{\sqrt{2}}\right)^2|V_\text{ud}|^2 \frac{64 \hat{t}^2}{(p^2+M_W^2)^2 +M_W^2\Gamma_W^2}.
\end{equation}
