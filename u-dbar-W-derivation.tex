...
%since \usepackage{physics} does not support linux
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{u-antid-wplus_v3}
	\caption{Feynman diagram representing the annihilation of an up quark with an anti-down quark, creating a $W^+$ boson.}
	\label{fig:u-antid-wplusv3}
\end{figure}
%wierdly, all .eps graphics must be in the exact same dir as the .tex file 
% it cannot parse .eps if its a folder (i.e. graphics) even if you list the full dir path
...
\end{itemize}	
 ...
 \subsection{Summing over quark spin states}\label{completeness}
...
\subsection{Summing over final polarisation states}\label{pol}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
 The contraction of $g_{\mu\nu}\epsilon^{\mu \nu \rho \sigma}p_{1\rho}p_{2\sigma}$ goes to zero because the Levi-Civita is totally antisymmetric and the metric tensor is totally symmetric under the exchange of indices, i.e. exchanging the $\mu$ and $\nu$ indicies gives,
...
\subsection{Spin and colour averages}
For an unpolarised beam, the initial spin is not known and so one must average over all possible spin states. The incoming up quark and anti-down quark each can have spin +$\frac{1}{2}$ or $-\frac{1}{2}$, thus, the average is $\frac{1}{4}$. In the same manner, one averages over initial colour states by dividing by the number of initial colour states.  The up and anti-down quakr each have three possible colour states, ($r$,$g$,$b$) and ($\bar{r}$,$\bar{g}$,$\bar{b}$) respectively. The naive colour averaged factor is $\frac{1}{9}$. Since, the weak interaction is a colour singlet, a red quark can only annihilate with an anti-red, green with anti-green, and blue with anti-blue. The initial colour degree of freedom is reduced to 3 and the colour averaged factor is now $\frac{1}{3}$. One can express the amplitude in terms of the mass of the W boson, $M_W$, via the relation between $g$ and the Fermi constant, $G_F$,
...
\begin{gather*}
	...
	\int d^2b \left(
	\int \frac{d^3k_A}{(2\pi)^3}\frac{\phi_A(\boldsymbol{k}_A)}{\sqrt{2E_A}}
	\int \frac{d^3k_B}{(2\pi)^3}\frac{\phi_B(\boldsymbol{k}_B)}{\sqrt{2E_B}}
	\int \frac{d^3\overline{k}_A}{(2\pi)^3}\frac{\phi_A^*(\overline{\boldsymbol{k}}_A)}{\sqrt{2\overline{E}_A}}
	\int \frac{d^3\overline{k}_B}{(2\pi)^3}\frac{\phi_B^*(\overline{\boldsymbol{k}}_B)}{\sqrt{2\overline{E}_B}}
	\right)\\
	\times e^{i\boldsymbol{b}\cdot(\overline{\boldsymbol{k}}_B-\boldsymbol{k}_B)}
	\left|\langle\boldsymbol{p}_f|\boldsymbol{k}_A\rangle\right|^2 \;
	\left|\langle\boldsymbol{p}_f|\boldsymbol{k}_B\rangle\right|^2 ,
\end{gather*}
...
\begin{equation}
	\begin{gathered}\label{final}
		...
		\times e^{i\boldsymbol{b}\cdot(\overline{\boldsymbol{k}}_B - \boldsymbol{k}_B)} \bra{\boldsymbol{p}_i}\ket{\boldsymbol{k}_i}\; \bra{\boldsymbol{p}_f}\ket{\overline{\boldsymbol{k}_i}}^*. 
	\end{gathered}
\end{equation}
...
\begin{gather*}
		...
		\cdot |\phi_A(\boldsymbol{k}_A)|^2\;|\phi_B(\boldsymbol{k_B})|^2 (2\pi)^4 \delta^4(k_A +k_B -\Sigma p_f).
\end{gather*}
...
\begin{equation}\label{dsigma}
		d\sigma  =\frac{1}{2} \frac{\sqrt{2}}{3}G_F M_W^2 (2\pi)^4 \delta^4(p_u +p_{\bar{d}} - p_W) \frac{d^3\boldsymbol{p}_W}{(2\pi)^32E_W},
\end{equation}
where $\hat{s}$ cancels out. To get the total cross-section, one has to integrate over the phase space.

$$\sigma = \int d\sigma = C \int \delta^4(p_u +p_{\bar{d}} - p_W) d^3\boldsymbol{p}_W, $$
...
$$\sigma =\frac{1}{2}\frac{\sqrt{2}}{3}G_FM^2_W(2\pi)^4\frac{1}{(2\pi)^32E_W}2M_W\delta(\hat{s} - M_W), $$
...
\begin{equation}
	\sigma =\frac{\pi \sqrt{2}}{3}G_FM^2_W\delta(\hat{s} - M_W).
\end{equation}
...
%\begin{figure}[h]
%	\centering
%	\includegraphics[width=0.7\linewidth]{graphics/W-decay-feyn}
%	\caption{Feynman diagram representing the decay of $W^+$ to an positron and electron-neutrino.}
%	\label{fig:w-decay-feyn}
%\end{figure}
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{W-decay-feyn}
	\caption{temp}
	\label{fig:w-decay-feyn}
\end{figure}
...
The squared amplitude can be simplified by using the relation $\bar{u} = u^{\dagger}\gamma^0$ and the identity from \ref{gamma_id}:
...
$$|\mathcal{M}|^2 = \frac{1}{4} \frac{g^2}{2} (\epsilon_\mu^* \bar{v}[\overline{\gamma^\mu (1-\gamma^5)}]u)(\bar{u}[\gamma^\nu (1-\gamma^5] v \epsilon_\nu). $$
 ...
 Once again, $\epsilon_{\mu}^*$ and $\epsilon_{\nu}$ will be treated separately later and will not contribute in the summation over spins. The notation for spin- and polarisation-summed amplitude squared will br split into the summation over spins component, denoted by $N^{\mu\nu}$, and summation over polarisation, denoted $Z_{\mu \nu}$, where, $$\sum_{s_1, s_2, \text{pol}}|\mathcal{M}|^2=  \frac{1}{4} \frac{g^2}{2} Z_{\mu\nu}N^{\mu\nu} $$\\
 ...
 where the identity \ref{overline-id} was used to convert $\overline{\gamma^\mu (1-\gamma^5)}$ back into $\gamma^\mu (1-\gamma^5).$ One can use the projector representation of $(1-\gamma^5)$ as $P_L = \frac{1}{2}(1-\gamma^5)$, and its properties to omit the mass terms in the expression, 
 ...
 \subsection{Summing over initial polarisation states}
...
\begin{equation}
	\begin{aligned}
			\sum_{s_1, s_2, \text{pol}}|\mathcal{M}|^2&=  \frac{1}{4} \frac{g^2}{2} Z_{\mu\nu}N^{\mu\nu}\\
			&= \frac{1}{4} \frac{g^2}{2}   \left(-g_{\mu\nu} + \frac{k_\mu k_\nu}{M_W^2}\right) 8[p_4^\mu p_3^\nu + p_4^\nu p_3^\mu - (p_3 \cdot p_4) g^{\mu \nu}]\\
			&= \frac{1}{3}g^2 \hat{s}
	\end{aligned}
\end{equation}
where the factor of 1/3 in the last line came from averaging over initial polarisation states for $W^+.$


%Barger and Phillips turn s hat into M_W squared; maybe to enforce energy-momentum conservation.


\subsection{Calculating decay rates}
One can derive the differential decay rate, $d \Gamma$, via a small modification to the general expression of the differential cross-section (Eq. \ref{general_cross}). Specifically, one can remove factors that involve a multi-particle initial state; the decaying particle --- let's say particle A --- is assumed to be at rest, so the factor 1/E$_A$ becomes 1/$m_A$. Thus the differential decay rate formula is,

\begin{equation}
	d\Gamma= \frac{|\mathcal{M}|^2 }{2m_A} d\Phi_{n_f},
\end{equation}
where,
...
$$
d\Gamma (W^+ \rightarrow e^+ \nu_e) =  \frac{1}{2M_W} \left(\frac{1}{3}g^2 \hat{s}\right) d\Phi_{2}.
$$

% since we summed over every polariation state , decay can be approximated to isotropic? YES, for a sample of unpolarised W bosons, decay is isotropic -- page 465 in slides has good explanations

% use the slides on WZ.ppt
